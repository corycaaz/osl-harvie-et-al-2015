...
This dataset is not tidy. Each row should contain one observation and each column one variable, intersecting at exactly one value. It is evident that we lack a response variable. What exactly is the outcome?

Remember, Harvie et al. (2015) measured the degree of rotation at which participants began to experience pain. In other words, the three feedback variables are measurements of the onset of pain. Since the `accurate_feedback` variable is constrained to 1, we only care about the onset of pain for the other two types of visual feedback.
...
We need to set a few of the variables as factors.
